#!/usr/bin/env python3
"""Run on the VPS: python3 proxy_diag.py https://www.youtube.com/shorts/VIDEO_ID
Pinpoints where the proxy breaks: youtube.com (extraction) vs googlevideo.com (media)."""
...
def curl(label, url, via_proxy):
    cmd = ["curl", "-sS", "-o", "/dev/null", "-m", "20", "-r", "0-1023",
           "-w", "%{http_code} tls=%{ssl_verify_result}", url]
    if via_proxy:
        cmd[1:1] = ["-x", proxy]
...
for vp in (True, False):
    ...
    curl(f"{tag} {host[:20]}.. generate_204", f"https://{host}/generate_204", vp)
