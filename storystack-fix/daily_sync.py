...
t0 = time.time()
channels = [r["id"] if isinstance(r, dict) else r[0]
            for r in db.all("SELECT id FROM channels")] if hasattr(db, "all") else None
if channels is None:
    import sqlite3
    channels = [r[0] for r in sqlite3.connect("/var/lib/storystack/storystack.db")
                .execute("SELECT id FROM channels")]
...
if key:
    import sqlite3
    ids = [r[0] for r in sqlite3.connect("/var/lib/storystack/storystack.db")
           .execute("SELECT id FROM videos")]
